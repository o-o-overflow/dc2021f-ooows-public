...
class VmmWorker():
    ...
    def sharepath(self):
        return os.path.join(VM_STORE_DIR, self.name, P9SHARE)

    def shared_files(self, path='.'):
        request = os.path.join(self.sharepath(), path)
        for root, dirs, files in os.walk(self.sharepath()):
            if root == os.path.realpath(request):
                fullpaths = map(lambda p: os.path.join(root, p), files)
                sizes = map(os.path.getsize, fullpaths)
