...
def files(vmid):
    ...
    return render_template("files.html", vm=vm,
                           files=shared_files)

@app.route('/getfile/<int:vmid>/<string:path>')
def getfile(vmid, path):
    ...
    return send_from_directory(directory=share, path=path)
